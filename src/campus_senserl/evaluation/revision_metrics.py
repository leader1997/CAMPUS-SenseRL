...
def standardize_eval(
    ...
    *,
    ...
    return {
        ...
        "recall": recall,
        "event_recall": recall,
        "event_recall_union": recall,
        "event_precision_union": precision,
        "event_f1_union": f1,
        "f1": f1,
        "attempt_rate": float(tx_rate),
        "delivery_rate": float(n_delivered / n_available) if np.isfinite(n_available) and n_available > 0 and np.isfinite(n_delivered) else float("nan"),
        "attempt_based_reduction": float(reduction),
        "aoi_raw": aoi,
        "mae_ok": mae_ok,
        "event_ok": event_ok,
        "aoi_ok": aoi_ok,
